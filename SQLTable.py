...
class SQLTable:
    ...
    def execute_query_with_params(self, query, params):
        """
        Executes a SQL query with parameters and returns the result as a DataFrame.
    ...
    def plot_keyword_trends_index(self, keywords, start_date, end_date, text_column='full_text', date_column='date'):
        ...
        pivot_df = result_df.pivot(index='month', columns='keyword', values='count').reindex(index=month_index).fillna(
            0).astype(int)
    ...
    def plot_keyword_trends(self, keywords, start_date, end_date, text_column='full_text', date_column='date'):
        """
        Подсчёт частоты ключевых слов по месяцам без использования полнотекстового индекса.
        :param keywords: список ключевых слов
        :param start_date: дата начала (в формате 'YYYY-MM-DD')
        :param end_date: дата окончания (в формате 'YYYY-MM-DD')
        :param text_column: имя столбца с текстом
        :param date_column: имя столбца с датой публикации
        ...
        for month_start in all_months:
            ...
            WHERE `{date_column}` BETWEEN %s AND %s
            """
            df = self.execute_query_with_params(query,
    ...
    def inspect_table_dates(self, limit=100):
        """
        Просматривает уникальные даты в таблице, чтобы понять распределение данных.
        :param limit: максимальное количество строк для вывода
        """
